...
func NewRedisGenericCache[K comparable, V genericports.ObjectWithIdentifier[K]](addr string, password string, db int) *RedisGenericCache[K, V] {
	client := redis.NewClient(&redis.Options{
		Addr:     addr,
		Password: password,
		DB:       db,
	})
	return &RedisGenericCache[K, V]{client: client}
}
